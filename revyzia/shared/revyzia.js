...
function loadFirebaseSDK(onReady, onError) {
  ...
  s1.onload = () => {
    ...
    s2.onload = () => { initFirebase(); onReady && onReady(); };
    s2.onerror = () => { onError && onError('database SDK failed'); };
    document.head.appendChild(s2);
  };
  s1.onerror = () => { onError && onError('app SDK failed'); };
...
function pushToFirebase(data, user) {
  ...
  try {
    ...
    fbDb.ref('users/' + key).set({
      name: data.name || user.name,
      userClass: data.userClass || user.klass,
      totalXp: data.totalXp || 0,
      streakDays: data.streakDays || 0,
      streakValue: data.streakValue || 0,
